...
DT            = 1.0
CHUNK_MINUTES = 60
CHUNK_SAMPLES = CHUNK_MINUTES * 60   # 3600
...
# ---------------------------------------------------------------------------
# Hyperparameter search space  (identisch mit 5-min Variante)
# ---------------------------------------------------------------------------
PARAM_GRID = {
    "sigma"                 : [0],
...
def get_valid_chunks(data: pd.DataFrame,
    ...
    """
    logger.info("Splitting into %d-sample (= %d min) chunks ...",
                chunk_samples, chunk_samples // 60)
    n_total  = len(data)
    ...
    logger.info("Found %d valid chunks.", len(all_chunks))

    if len(all_chunks) == 0:
        raise ValueError("No valid chunks found. Dataset may be too small for 60-min chunks.")
...
def main():
    parser = argparse.ArgumentParser(
        description="PySR HP Tuning – Synthetic 60-min chunks"
    )
    parser.add_argument("--data-path", type=str,
                        default="/home/ka/ka_iai/ka_hr7224/synthetic_dataset_validation/synthetic_data_noiseless.pkl")
    parser.add_argument("--results-dir", type=str,
                        default="/home/ka/ka_iai/ka_hr7224/synthetic_dataset_validation/results_hp_tuning/60min")
    ...
    logger.info("=" * 60)
    logger.info("PySR HP TUNING  –  60-min chunks  –  SYNTHETIC")
