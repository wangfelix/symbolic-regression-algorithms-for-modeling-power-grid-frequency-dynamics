#!/bin/bash
#SBATCH --job-name=sindy_hp_1h
#SBATCH --time=48:00:00
#SBATCH --partition=cpu
#SBATCH --output=./slurm-logs-synthetic-hp-tuning-1h/%x-%A_%a.out
...
# Run the SINDy hyperparameter tuning script (executing all 1-hour chunks per combo)
python -u run_sindy_synthetic_hp_tuning_1h.py --run-name ${RUN_NAME} --combo-index $SLURM_ARRAY_TASK_ID
