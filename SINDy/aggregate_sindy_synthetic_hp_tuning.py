"""
Aggregate results from SINDy hyperparameter tuning on synthetic data.

Reads all per-combo CSV files from results_sindy_synthetic_hp_tuning/<run-name>/,
...
Works for both 5-min and 1-hour chunk results via --results-dir.

Usage:
    python aggregate_sindy_synthetic_hp_tuning.py --run-name run_SLURM_4011902
    python aggregate_sindy_synthetic_hp_tuning.py --run-name run_SLURM_4011904 --results-dir results_sindy_synthetic_hp_tuning_1h
"""
import os
import glob
import pandas as pd
...
def main():
    parser = argparse.ArgumentParser(
        description="Aggregate parallel SLURM array SINDy synthetic HP tuning runs."
    )
    parser.add_argument(
        "--run-name", type=str, required=True,
        help="Folder name inside results dir, e.g. run_SLURM_4011902"
    )
    parser.add_argument(
        "--results-dir", type=str, default="results_sindy_synthetic_hp_tuning",
        help="Results parent folder name (default: results_sindy_synthetic_hp_tuning). "
             "Use results_sindy_synthetic_hp_tuning_1h for 1-hour chunks."
    )
    args = parser.parse_args()

    results_dir = os.path.join(
        os.path.dirname(__file__), args.results_dir, args.run_name
    )
    ...
    # ---- Save master CSV ----
    master_csv_path = os.path.join(results_dir, f"sindy_synthetic_hp_tuning_{args.run_name}_all.csv")
    ...
    # ---- Print summary ----
    print(f"\n{'=' * 70}")
    print(f"SINDy SYNTHETIC HP TUNING — AGGREGATED RESULTS ({len(combined_df)} COMBOS)")
    print(f"{'=' * 70}")
    print(f"Results directory: {args.results_dir}")
    print(f"Combos with >50% success rate: {len(valid_df)}/{len(combined_df)}")
    if empty_count > 0:
        print(f"Empty combo files (no results): {empty_count}")
    ...
    if "Mean_RMSE_Theta" in best_row:
        print(f"  Mean RMSE Theta:    {best_row['Mean_RMSE_Theta']:.6f}")
    if "Mean_RMSE_Total" in best_row:
        print(f"  Mean RMSE Total:    {best_row['Mean_RMSE_Total']:.6f}")
    ...
    # ---- Bottom-10 table (worst combos) ----
    bottom10 = valid_df.nlargest(10, "Mean_RMSE_Omega")
    print(f"\nBottom-10 Combos by Mean_RMSE_Omega (worst):")
    print(f"{'Rank':<6}{'Combo':<8}{'Sigma':<8}{'Degree':<8}{'Threshold':<14}{'Mean_RMSE':<14}{'Median_RMSE':<14}{'Success':<10}")
    print("-" * 82)
    ...
    summary = {
        ...
        "best_combo": {
            ...
            "mean_rmse_theta": float(best_row.get("Mean_RMSE_Theta", float('nan'))),
            "mean_rmse_total": float(best_row.get("Mean_RMSE_Total", float('nan'))),
