"""
SINDy Hyperparameter Tuning on Synthetic Noisy Dataset

Exhaustive grid search over sigma, degree, and STLSQ threshold using
the synthetic dataset with wiener noise in 5-minute chunks.

Usage:
    python run_sindy_synthetic_hp_tuning.py --combo-index 0
    python run_sindy_synthetic_hp_tuning.py --combo-index 0 --run-name run_SLURM_12345
    python run_sindy_synthetic_hp_tuning.py  # evaluate all combos sequentially
    python run_sindy_synthetic_hp_tuning.py --dry-run
"""
import os
import sys
import numpy as np
...
def main():
    ...
    print(f"\n{'=' * 60}")
    print(f"SINDy HYPERPARAMETER TUNING — EXHAUSTIVE GRID SEARCH (SYNTHETIC)")
    print(f"{'=' * 60}")
    ...
    # Output directory
    if args.run_name:
        results_dir = os.path.join(os.path.dirname(__file__), "results_sindy_synthetic_hp_tuning", args.run_name)
    else:
        results_dir = os.path.join(os.path.dirname(__file__), "results_sindy_synthetic_hp_tuning", f"run_{timestamp}")
