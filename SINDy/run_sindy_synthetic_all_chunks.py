"""
SINDy Analysis: All 5-Minute Chunks from Synthetic Noisy Dataset

Fits a PySINDy model with best hyperparameters from tuning on each
5-min chunk of the synthetic noisy dataset.
Designed to run as part of a SLURM array job.

Usage:
    python run_sindy_synthetic_all_chunks.py --start-chunk 0 --end-chunk 999
    python run_sindy_synthetic_all_chunks.py  # all chunks (single node)
"""
import os
import sys
import numpy as np
...
# =============================================================================
# Best Configuration (from HP tuning on synthetic_with_wiener)
# =============================================================================
SIGMA = 15
DEGREE = 1
STLSQ_THRESHOLD = 1e-10
# Source: HP tuning run_SLURM_4011902, combo 45
# Mean RMSE omega=0.039248, Median=0.027585
...
def fit_single_chunk(chunk):
    ...
    try:
        ...
        # For degree=1: [1, theta, omega] -> 3 terms
...
def main():
    parser = argparse.ArgumentParser(description="SINDy analysis on all synthetic 5-min chunks")
    ...
    print(f"\n{'=' * 60}")
    print(f"SINDy ANALYSIS — SYNTHETIC NOISY (5-MIN CHUNKS)")
    print(f"{'=' * 60}")
    ...
    # Output directory
    results_base = os.path.join(os.path.dirname(__file__), "results_sindy_synthetic_all_chunks")
    if args.run_name:
    ...
    print(f"\n{'=' * 60}")
    print(f"SINDy ANALYSIS COMPLETE — Synthetic Noisy [{start}, {end}]")
    print(f"{'=' * 60}")
