"""
Plot SVISE Results (5-minute chunks)

This script generates comparison plots between empirical data and the learned SDE model.
It reads from results_5min/<timestamp>/model.json and saves plots to results_5min/<timestamp>/plots/.
The chunk index is read from the model.json config so the correct 5-minute chunk is used for comparison.

Usage:
    python plot_results_5min_chunk.py --timestamp 20260223_214337
    python plot_results_5min_chunk.py --all  # Plot all available results
"""
...
def get_valid_chunk_5min(data, chunk_index=0):
    ...
    valid_chunks = [group for _, group in chunk_groups if len(group) == 300]
    
    if not valid_chunks:
        raise ValueError("No valid 5-minute chunks found.")
    
    if chunk_index < 0 or chunk_index >= len(valid_chunks):
        raise ValueError(f"Chunk index {chunk_index} out of range. Found {len(valid_chunks)} valid 5-minute chunks (0-{len(valid_chunks)-1}).")
    
    print(f"Found {len(valid_chunks)} valid 5-minute chunks. Using chunk {chunk_index}.")
    return valid_chunks[chunk_index]
...
def simulate_model_scaled(t, theta0, omega0, coeffs_omega, scaling_params, coeffs_theta=None):
    ...
    Model was learned on:
      X_scaled = (X - mean) / std
      dX_scaled/dt = f(X_scaled)
    
    Time scaling is optional (defaults to 1.0 = no scaling).
    """
    mean = np.array(scaling_params["mean_x"])
    std = np.array(scaling_params["std_x"])
    t_scale = scaling_params.get("t_scale", 1.0)
    ...
    # Scale time (no-op when t_scale=1.0)
    t_scaled = t / t_scale
...
def plot_results(timestamp, results_dir, data_path):
    ...
    # Read chunk index from config
    chunk_index = config.get("chunk", 0)
    
    print(f"Plotting results for timestamp: {timestamp}")
    print(f"Model: {config.get('model', 'unknown')}, Sigma: {config.get('sigma', 5)}, Chunk: {chunk_index}")
    
    # Load empirical data
    data = load_data(data_path)
    chunk_df = get_valid_chunk_5min(data, chunk_index=chunk_index)
    ...
    axes[0].set_title(f'SVISE Model vs Empirical Data (5min chunk #{chunk_index}, Timestamp: {timestamp})', fontsize=14, y=1.2)
    ...
    eq_text += f"Sigma: {config.get('sigma', 5)}, Degree: {config.get('degree', 2)}, Chunk: {chunk_index}\n"
...
def main():
    parser = argparse.ArgumentParser(description="Plot SVISE results (5-minute chunks)")
    parser.add_argument("--timestamp", type=str, default=None,
                        help="Timestamp of the result to plot (e.g., 20260223_214337)")
    ...
    results_dir = os.path.join(os.path.dirname(__file__), "results_5min")
    ...
    if args.all:
        ...
        if timestamps:
            ...
            print("No results found. Run run_analysis_5min_chunks.py first.")
