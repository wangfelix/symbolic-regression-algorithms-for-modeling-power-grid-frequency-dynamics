...
def get_valid_chunk_5min(data, chunk_index=0):
    ...
    # Filter for full chunks (300 samples at 1Hz)
...
def run_experiment(args):
    ...
    chunk_df = get_valid_chunk_5min(data, chunk_index=args.chunk)
    ...
    # Time scaling: SVISE's GP quadrature requires t in a small range (~0-10).
    # 300 seconds / 30 = 10 (same normalized range as 15-min pipeline: 900/90=10)
    t_scale = 30.0
...
def save_experiment_results(args, loss, equations, start_time, loss_history=None, scaling_params=None, rmse_results=None):
    ...
    log_dir = os.path.join(os.path.dirname(__file__), "results_5min")
    ...
    # Append to CSV
    csv_path = os.path.join(os.path.dirname(__file__), "experiments_log_5min.csv")
    ...
    with open(csv_path, 'a', newline='') as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(["Timestamp", "Model", "Sigma", "Degree", "Tau", "Chunk", "LR", "Epochs", "FinalLoss", "RMSE_Theta", "RMSE_Omega", "RMSE_Total", "Equation_Theta", "Equation_Omega"])
...
def main():
    parser = argparse.ArgumentParser(description="Run SVISE Analysis on Power Grid Data (5-minute chunks)")
    ...
    parser.add_argument("--chunk", type=int, default=0,
                        help="Index of the 5-minute chunk to use (0-indexed)")
