"""
SVISE Final Evaluation: All Active 1-Hour Chunks from Synthetic Noiseless Data.
...
Usage:
    python run_svise_synthetic_noiseless_1h.py --start-chunk 0 --end-chunk 99
    python run_svise_synthetic_noiseless_1h.py  # all chunks
"""
import os
import sys
import pandas as pd
...
# Source: HP tuning run_SLURM_3718590, combo 56 (5-min tuning)
...
def main():
    ...
    print(f"\n{'=' * 60}")
    print(f"SVISE FINAL EVALUATION - SYNTHETIC NOISELESS (1-HOUR CHUNKS)")
    print(f"{'=' * 60}")
    ...
    # Output directory
    results_base = os.path.join(script_dir, "results_synthetic_noiseless_1h")
    if args.run_name:
    ...
    print(f"\n{'=' * 60}")
    print(f"EVALUATION COMPLETE - Synthetic Noiseless 1h [{start}, {end}]")
    print(f"{'=' * 60}")
