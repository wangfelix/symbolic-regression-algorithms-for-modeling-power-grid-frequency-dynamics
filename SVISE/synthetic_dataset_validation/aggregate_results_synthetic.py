...
Reads all per-chunk CSV files from results_synthetic_noiseless/<run-name>/,
...
Usage:
    python aggregate_results_synthetic.py --run-name run_SLURM_3726261
"""
import os
import re
import glob
...
def main():
    ...
    parser.add_argument("--run-name", type=str, required=True,
                        help="Folder name inside results_synthetic_noiseless, e.g. run_SLURM_3726261")
    args = parser.parse_args()

    script_dir = os.path.dirname(os.path.abspath(__file__))
    results_dir = os.path.join(script_dir, "results_synthetic_noiseless", args.run_name)
    ...
    W = 75
    print(f"\n{'=' * W}")
    print(f"AGGREGATED RESULTS - SYNTHETIC NOISELESS - {stats['successful_chunks_gp']} CHUNKS")
    print(f"{'=' * W}")
