"""
SVISE Final Evaluation: All Active 1-Hour Chunks from Synthetic Noisy (with Wiener) Data.

Same as run_svise_synthetic_noiseless.py but with 1-hour (3600 sample) chunks
to match Wen et al.'s original chunk size.

Designed for SLURM array jobs.

Usage:
    python run_svise_synthetic_noisy_1h.py --start-chunk 0 --end-chunk 99
    python run_svise_synthetic_noisy_1h.py  # all chunks
"""
import os
import sys
import pandas as pd
...
# =============================================================================
# BEST HYPERPARAMETERS (same as 5-min; update if 1h HP tuning is done)
# =============================================================================
BEST_HYPERPARAMS = {
    "model": "integrator",
    "sigma": 0,
...
# Source: HP tuning run_SLURM_4011899 on synthetic_with_wiener (1h chunks)

# Chunk configuration
CHUNK_SIZE = 3600        # 1 hour at 1s resolution
T_SCALE = 360.0          # Time scaling: 3600/360 = 10 (same scaled range as 5-min)
...
def main():
    parser = argparse.ArgumentParser(description="SVISE evaluation on all active synthetic 1-hour chunks")
    ...
    print(f"\n{'=' * 60}")
    print(f"SVISE FINAL EVALUATION - SYNTHETIC NOISY (1-HOUR CHUNKS)")
    print(f"{'=' * 60}")
    ...
    # Output directory
    results_base = os.path.join(script_dir, "results_synthetic_noisy_1h")
    if args.run_name:
    ...
    print(f"\n{'=' * 60}")
    print(f"EVALUATION COMPLETE - Synthetic Noisy (with Wiener) 1h [{start}, {end}]")
    print(f"{'=' * 60}")
